...
#include <cpu_func.h>
#include <dt-bindings/soc/ky-x1.h>
...
extern int x1_eeprom_init(void);
extern int ky_eeprom_read(uint8_t *buffer, uint8_t id);
...
static uint32_t adjust_cpu_freq(uint64_t cluster, uint32_t freq)
{
	...
	val = readl((void __iomem *)(X1_APMU_BASE + 0x38c + cluster*4));
	val &= ~(0x07 | BIT(13));
	switch(freq) {
	case 1600000:
		val |= 0x07;
	...
	writel(val, (void __iomem *)(X1_APMU_BASE + 0x38c + cluster*4));

	/* set cluster frequency change request, and wait done */
	val = readl((void __iomem *)(X1_APMU_BASE + 0x38c + cluster*4));
	val |= BIT(12);
	writel(val, (void __iomem *)(X1_APMU_BASE + 0x38c + cluster*4));
	while(readl((void __iomem *)(X1_APMU_BASE + 0x38c + cluster*4)) & BIT(12));
...
void raise_cpu_frequency(void)
{
	...
	/* enable CLK_1228M */
	val = readl((void __iomem *)(X1_MPMU_BASE + 0x1024));
	val |= BIT(16) | BIT(15) | BIT(14) | BIT(13);
	writel(val, (void __iomem *)(X1_MPMU_BASE + 0x1024));

	/* enable PLL3(3200Mhz) */
	val = readl((void __iomem *)(X1_APB_SPARE_BASE + 0x12C));
	val |= BIT(31);
	writel(val, (void __iomem *)(X1_APB_SPARE_BASE + 0x12C));
	/* enable PLL3_DIV2 */
	val = readl((void __iomem *)(X1_APB_SPARE_BASE + 0x128));
	val |= BIT(1);
	writel(val, (void __iomem *)(X1_APB_SPARE_BASE + 0x128));
...
int board_fit_config_name_match(const char *name)
{
	...
	/*
		...
		such as: k1_deb1 -> x1_deb1
	*/
	if (!strncmp(buildin_name, "k1_", 3)){
		sprintf(tmp_name, "%s_%s", "x1", &buildin_name[3]);
...
{
	if ((x1_eeprom_init() >= 0) && (NULL != mac_addr) &&
		(0 == ky_eeprom_read((uint8_t*)mac_addr, TLV_CODE_MAC_BASE))) {
...
char *get_product_name(void)
{
	...
	name = calloc(1, 64);
	if ((x1_eeprom_init() >= 0) && (NULL != name) &&
		(0 == ky_eeprom_read(name, TLV_CODE_PRODUCT_NAME))) {
...
void update_ddr_info(void)
{
	...
	info = malloc(32);
	memset(info, 0, 32);
	if (0 == ky_eeprom_read(info, TLV_CODE_DDR_TYPE))
		ddr_type = info;
	else
		free(info);

	// if fail to get ddr cs number from eeprom, update it from dts node
	if (0 == ky_eeprom_read((uint8_t*)&ddr_cs_num, TLV_CODE_DDR_CSNUM))
		pr_info("Get ddr cs num %d from eeprom\n", ddr_cs_num);
	else
		ddr_cs_num = 0;

	// if fail to get ddr cs number from eeprom, update it from dts node
	if (0 == ky_eeprom_read((uint8_t*)&ddr_datarate, TLV_CODE_DDR_DATARATE)) {
