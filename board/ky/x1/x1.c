...
void import_env_from_bootfs(void)
{
	...
	case BOOT_MODE_NAND:
		...
		sprintf(cmd, "env import -t 0x%x", CONFIG_SPL_LOAD_FIT_ADDRESS);
		if (!run_command(cmd, 0)) {
			pr_err("Imported environment from 'env_x1.txt'\n");
		}
#endif
		break;
	case BOOT_MODE_NOR:
...
#endif
#ifdef CONFIG_KY_SHUTDOWN_CHARGE
	struct udevice *udev;
...
int board_fit_config_name_match(const char *name)
{
	...
	*/
	if (!strncmp(product_name, "k1_", 3)){
		sprintf(tmp_name, "%s_%s", "x1", &product_name[3]);
...
int ft_board_setup(void *blob, struct bd_info *bd)
{
	...
	static const struct node_info nodes[] = {
		{ "ky,x1-qspi", MTD_DEV_TYPE_NOR, },  /* SPI flash */
	};
