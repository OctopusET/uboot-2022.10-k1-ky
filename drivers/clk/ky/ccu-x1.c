...
//apbs
static KY_CCU_FACTOR(pll1_2457p6_vco, "pll1_2457p6_vco", "pll1_vco",
	1, 100);
...
static struct ky_clk_table ky_x1_clks = {
	.clks	= {
...
static KY_CCU_DDN_GATE(slow_uart1_14p74, "slow_uart1_14p74", "pll1_d16_153p6",
	&uart_ddn_mask_info, &slow_uart1_tbl, ARRAY_SIZE(slow_uart1_tbl),
	BASE_TYPE_MPMU, MPMU_SUCCR, MPMU_ACGR, BIT(1),
	0);
static KY_CCU_DDN_GATE(slow_uart2_48, "slow_uart2_48", "pll1_d4_614p4",
...
static KY_CCU_DIV_MFC_MUX_GATE(qspi_clk, "qspi_clk", qspi_parent_names,
	BASE_TYPE_APMU, APMU_QSPI_CLK_RES_CTRL,
	9, 3, BIT(12),
	6, 3, BIT(4), BIT(4), 0x0,
	0);
...
static KY_CCU_GATE_NO_PARENT(emac0_bus_clk, "emac0_bus_clk", NULL,
	...
	BIT(0), BIT(0), 0x0,
	0);
static KY_CCU_GATE(emac0_ptp_clk, "emac0_ptp_clk", "pll1_d3_819p2",
	BASE_TYPE_APMU, APMU_EMAC0_CLK_RES_CTRL,
	BIT(15), BIT(15), 0x0,
	0);
static KY_CCU_GATE_NO_PARENT(emac1_bus_clk, "emac1_bus_clk", NULL,
	...
	BIT(0), BIT(0), 0x0,
	0);
static KY_CCU_GATE(emac1_ptp_clk, "emac1_ptp_clk", "pll1_d3_819p2",
	BASE_TYPE_APMU, APMU_EMAC1_CLK_RES_CTRL,
	BIT(15), BIT(15), 0x0,
	0);
...
struct ky_clk_init_rate init_rate_tbl[] = {
#ifdef CONFIG_SPL_BUILD
...
ulong ccu_clk_get_rate(struct clk *clk)
{
	...
	struct clk *c = ky_x1_clks.clks[clk->id];
	if (!clk_valid(c))
		return 0;
...
int ccu_clk_set_parent(struct clk *clk, struct clk *parent)
{
	...
	struct clk *c = ky_x1_clks.clks[clk->id];
	struct clk *p = ky_x1_clks.clks[parent->id];
	if (!clk_valid(c))
		return 0;
	ops = ccu_clk_dev_ops(c->dev);
	if(ops->set_parent)
...
int ccu_common_init(struct clk * clk, struct ky_x1_clk *clk_info, struct ky_clk_table *clks)
{
...
int ky_ccu_probe(struct ky_x1_clk *clk_info,
		    struct ky_clk_table *clks)
{
	int i;
...
#endif
		clk->id = id;
		ky_x1_clks.clks[id] = clk;
	}
}

static int ky_x1_ccu_probe(struct udevice *dev)
{
	int ret = 0;
	struct ky_x1_clk *clk_info = &x1_clock_controller;
	struct ky_clk_table *clks = &ky_x1_clks;
	...
	ret = ky_ccu_probe(clk_info, clks);
...
static const struct udevice_id ccu_clk_ids[] = {
	{ .compatible = "ky,x1-ccu" },
	{ },
};

U_BOOT_DRIVER(ky_x1_ccu) = {
	.name = "x1-ccu",
	.id = UCLASS_CLK,
	.of_match = ccu_clk_ids,
	.ops = &ccu_clk_ops,
	.probe = ky_x1_ccu_probe,
};
