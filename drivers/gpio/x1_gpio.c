...
static void __iomem *x1_gpio_base;
#else
#define X1_GPIO_BASE   0xd4019000
#endif

#ifndef X1_MAX_GPIO
#define X1_MAX_GPIO	128
#endif
...
/**
 * struct x1_gpio_pctrl_map - gpio and pinctrl mapping
...
/**
 * struct x1_gpio_pctrl_map - gpio device instance
...
#ifdef CONFIG_DM_GPIO
	return (struct gpio_reg *)(x1_gpio_base + offset[bank]);
#else
	return (struct gpio_reg *)(X1_GPIO_BASE + offset[bank]);
#endif
}
...
#ifdef CONFIG_DM_GPIO
/**
 * x1_get_gpio_pctrl_mapping() - get associated pinctrl pin from gpio pin
 *
 * @plat: x1 GPIO device
 * @gpio: GPIO pin
 */
static int x1_get_pctrl_from_gpio(struct x1_gpio_plat *plat, u32 gpio, u32 *pctrl_pin)
{
	struct x1_gpio_pctrl_map *range = NULL;
	struct list_head *pos, *tmp;
	int ret = -EINVAL;

	list_for_each_safe(pos, tmp, &plat->gpiomap) {
		range = list_entry(pos, struct x1_gpio_pctrl_map, node);
...
/**
 * x1_get_gpio_pctrl_mapping() - get mapping between gpio and pinctrl
 *
 ...
 */
static int x1_get_gpio_pctrl_mapping(struct udevice *dev)
{
	struct x1_gpio_plat *plat = dev_get_plat(dev);
	struct x1_gpio_pctrl_map *range = NULL;
	...
	for (;; index++) {
		...
		range = (struct x1_gpio_pctrl_map *)devm_kzalloc(dev, sizeof(*range), GFP_KERNEL);
		if (!range)
			return -ENOMEM;
...
static int gpio_x1_probe(struct udevice *dev)
{
	struct x1_gpio_plat *plat = dev_get_plat(dev);
	...
	if (ret == 0) {
		...
		ret = x1_get_gpio_pctrl_mapping(dev);
		if (ret < 0) {
...
static const struct udevice_id gpio_x1_ids[] = {
	{ .compatible = "ky,x1-gpio" },
	{ }
};

static int x1_gpio_request(struct udevice *dev, unsigned gpio,
			     const char *label)
{
	struct x1_gpio_plat *plat = dev_get_plat(dev);
	u32 pctrl;
	...
	ret = x1_get_pctrl_from_gpio(plat, gpio, &pctrl);
	if (ret < 0)
		return 0;
...
static int x1_gpio_get_value(struct udevice *dev, unsigned int gpio)
{
	return _gpio_get_value(gpio);
}

static int x1_gpio_set_value(struct udevice *dev, unsigned int gpio,
				   int value)
{
	return _gpio_set_value(gpio, value);
}

static int x1_gpio_direction_input(struct udevice *dev, unsigned int gpio)
{
	return _gpio_direction_input(gpio);
}

static int x1_gpio_direction_output(struct udevice *dev, unsigned int gpio,
					  int value)
{
	return _gpio_direction_output(gpio, value);
}

static const struct dm_gpio_ops gpio_x1_ops = {
	.request		= x1_gpio_request,
	.direction_input	= x1_gpio_direction_input,
	.direction_output	= x1_gpio_direction_output,
	.get_value		= x1_gpio_get_value,
	.set_value		= x1_gpio_set_value,
};
U_BOOT_DRIVER(gpio_x1) = {
	.name	= "gpio_x1",
	.id	= UCLASS_GPIO,
	.ops	= &gpio_x1_ops,
	.of_match = gpio_x1_ids,
	.probe	= gpio_x1_probe,
	.plat_auto	= sizeof(struct x1_gpio_plat),
};
