...
{
	int ret;
	struct ky_efuse_plat *plat = dev_get_plat(dev);
...
static int ky_efuse_read(struct udevice *dev, int offset, void *buf, int size)
{
	return efuse_read_bank(dev, offset, buf, size);
}

static __maybe_unused int ky_efuse_program(struct udevice *dev, int offset, const void *buf, int size)
{
	int ret;
...
static const struct misc_ops ky_efuse_ops = {
	.read = ky_efuse_read,
#if !defined(CONFIG_SPL_BUILD)
	.write = ky_efuse_program,
#endif
};
...
static int ky_efuse_of_to_plat(struct udevice *dev)
{
	int ret;
	struct ky_efuse_plat *plat = dev_get_plat(dev);
...
static const struct udevice_id ky_efuse_ids[] = {
	{ .compatible = "ky,x1-efuse" },
	{}
};

U_BOOT_DRIVER(ky_x1_efuse) = {
	.name = "ky_x1_efuse",
	.id = UCLASS_MISC,
	.of_match = ky_efuse_ids,
	.of_to_plat = ky_efuse_of_to_plat,
	.plat_auto = sizeof(struct ky_efuse_plat),
	.ops = &ky_efuse_ops,
};
