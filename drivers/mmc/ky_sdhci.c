...
#define KY_SDHC_MIN_FREQ    (400000)
...
static const struct sdhci_ops ky_ops = {
...
static struct dm_mmc_ops ky_mmc_ops;
...
static int ky_sdhci_probe(struct udevice *dev)
{
	...
	host->mmc = &plat->mmc;
	host->mmc->dev = dev;
	ky_mmc_ops = sdhci_ops;
	ky_mmc_ops.reinit = sdhci_probe;

	ret = sdhci_setup_cfg(&plat->cfg, host, max_clk, KY_SDHC_MIN_FREQ);
	if (ret)
		return ret;
