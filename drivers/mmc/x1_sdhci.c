...
/*
 * Driver for Ky K1x Mobile Storage Host Controller
 *
...
struct ky_sdhci_priv {
	struct sdhci_host host;
	u32 clk_src_freq;
	u32 phy_module;
};

struct ky_sdhci_driver_data {
...
static void ky_mmc_phy_init(struct sdhci_host *host)
{
	struct udevice *dev = host->mmc->dev;
	struct ky_sdhci_priv *priv = dev_get_priv(dev);
...
int ky_set_sdh_74_clk(struct udevice *dev)
{
	struct ky_sdhci_priv *priv = dev_get_priv(dev);
...
void ky_sdhci_set_control_reg(struct sdhci_host *host)
{
...
static int ky_sdhci_phy_dll_init(struct sdhci_host *host)
{
	u32 reg;
	int i;
...
static int ky_sdhci_hs400_enhanced_strobe(struct sdhci_host *host)
{
	u32 reg;
	...
	return ky_sdhci_phy_dll_init(host);
}
#endif

static int ky_sdhci_probe(struct udevice *dev)
{
	struct ky_sdhci_driver_data *drv_data =
			(struct ky_sdhci_driver_data *)dev_get_driver_data(dev);
	struct ky_sdhci_plat *plat = dev_get_plat(dev);
	...
	mmc_driver_ops->deferred_probe = ky_set_sdh_74_clk;
	...
	ky_mmc_phy_init(host);

	pr_info("%s: probe done.\n", host->name);
	return ret;
}

static int ky_sdhci_of_to_plat(struct udevice *dev)
{
	struct ky_sdhci_plat *plat = dev_get_plat(dev);
	struct ky_sdhci_priv *priv = dev_get_priv(dev);
	struct sdhci_host *host = &priv->host;
...
static int ky_sdhci_bind(struct udevice *dev)
{
	struct ky_sdhci_plat *plat = dev_get_plat(dev);
...
const struct sdhci_ops ky_sdhci_ops = {
	.set_control_reg = ky_sdhci_set_control_reg,
#if CONFIG_IS_ENABLED(MMC_HS400_ES_SUPPORT)
	.set_enhanced_strobe = ky_sdhci_hs400_enhanced_strobe,
#endif
};

const struct ky_sdhci_driver_data ky_sdhci_drv_data = {
	.ops = &ky_sdhci_ops,
};

static const struct udevice_id ky_sdhci_ids[] = {
	{ .compatible = "ky,x1-sdhci",
	  .data = (ulong)&ky_sdhci_drv_data,
	},
	{ }
};

U_BOOT_DRIVER(ky_sdhci_drv) = {
	.name		= "ky_sdhci",
	.id		= UCLASS_MMC,
	.of_match	= ky_sdhci_ids,
	.of_to_plat = ky_sdhci_of_to_plat,
	.ops		= &sdhci_ops,
	.bind		= ky_sdhci_bind,
	.probe		= ky_sdhci_probe,
	.priv_auto = sizeof(struct ky_sdhci_priv),
	.plat_auto = sizeof(struct ky_sdhci_plat),
};
