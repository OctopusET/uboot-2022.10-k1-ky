...
/*
 * Ky x1 DesignWare based PCIe host controller driver
 *
...
static inline u32 x1_pcie_readl(struct pcie_dw_x1 *pcie, u32 offset)
{
	return readl(pcie->base + offset);
}

static inline void x1_pcie_writel(struct pcie_dw_x1 *pcie, u32 offset,
				      u32 value)
{
	writel(value, pcie->base + offset);
}

static inline u32 x1_pcie_phy_ahb_readl(struct pcie_dw_x1 *pcie, u32 offset)
{
	return readl(pcie->phy_ahb + offset);
}

static inline void x1_pcie_phy_ahb_writel(struct pcie_dw_x1 *pcie, u32 offset,
				      u32 value)
{
	writel(value, pcie->phy_ahb + offset);
}

static inline u32 x1_pcie_phy_reg_readl(struct pcie_dw_x1 *pcie, u32 offset)
{
	return readl(pcie->phy_addr + offset);
}

static inline void x1_pcie_phy_reg_writel(struct pcie_dw_x1 *pcie, u32 offset,
				      u32 value)
{
	writel(value, pcie->phy_addr + offset);
}

static inline u32 x1_pcie_conf0_reg_readl(struct pcie_dw_x1 *pcie, u32 offset)
{
	return readl(pcie->conf0_addr + offset);
}

static inline void x1_pcie_conf0_reg_writel(struct pcie_dw_x1 *pcie, u32 offset,
				      u32 value)
{
	writel(value, pcie->conf0_addr + offset);
}

static inline u32 x1_pcie_phy0_reg_readl(struct pcie_dw_x1 *pcie, u32 offset)
{
	return readl(pcie->phy0_addr + offset);
}

static inline void x1_pcie_phy0_reg_writel(struct pcie_dw_x1 *pcie, u32 offset,
				      u32 value)
{
 ...
 */
static void pcie_dw_configure(struct pcie_dw_x1 *pci, u32 cap_speed)
{
	u32 val;
 ...
 */
static int is_link_up(struct pcie_dw_x1 *pci)
{
	u32 val;
 ...
 */
static int wait_link_up(struct pcie_dw_x1 *pci)
{
...
static int pcie_dw_x1_pcie_link_up(struct pcie_dw_x1 *pci, u32 cap_speed)
{
	u32 reg;
	...
	reg = x1_pcie_readl(pci, PCIECTRL_X1_CONF_DEVICE_CMD);
	reg |= LTSSM_EN;
	reg &= ~APP_HOLD_PHY_RST;
	x1_pcie_writel(pci, PCIECTRL_X1_CONF_DEVICE_CMD, reg);
...
static int pcie_set_mode(struct pcie_dw_x1 *pci,
			       enum dw_pcie_device_mode mode)
{
	...
	case DW_PCIE_RC_TYPE:
		...
		reg = x1_pcie_readl(pci, PCIE_CTRL_LOGIC);
		reg |= PCIE_IGNORE_PERSTN;
		x1_pcie_writel(pci, PCIE_CTRL_LOGIC, reg);
		break;
...
static int x1_pcie_host_init(struct pcie_dw_x1 *pci)
{
	u32 reg;
...
void porta_rterm(struct pcie_dw_x1 *x1)
{
	int rd_data;
	u32 val;
...
void rterm_force(struct pcie_dw_x1 *x1, u32 pcie_rcal)
{
	int i, lane;
	u32 val = 0;
...
static int init_phy(struct pcie_dw_x1 *x1)
{
...
static int pcie_dw_init_id(struct pcie_dw_x1 *pci)
{
	dw_pcie_dbi_write_enable(&pci->dw, true);
	writew(X1_PCIE_VENDOR_ID, pci->dw.dbi_base + PCI_VENDOR_ID);
...
/**
 * pcie_dw_x1_probe() - Probe the PCIe bus for active link
 *
 ...
 */
static int pcie_dw_x1_probe(struct udevice *dev)
{
	struct pcie_dw_x1 *pci = dev_get_priv(dev);
	...
	x1_pcie_host_init(pci);
	pcie_dw_setup_host(&pci->dw);
	pcie_dw_init_id(pci);

	if (!pcie_dw_x1_pcie_link_up(pci, LINK_SPEED_GEN_2)) {
...
/**
 * pcie_dw_x1_of_to_plat() - Translate from DT to device state
 *
 * @dev: A pointer to the device being operated on
 *
 ...
 */
static int pcie_dw_x1_of_to_plat(struct udevice *dev)
{
	int ret = 0;
	struct pcie_dw_x1 *pcie = dev_get_priv(dev);
	...
	pcie->base = (void *)dev_read_addr_name(dev, "x1_conf");
	...
	ret = dev_read_u32(dev, "x1,pcie-port", &pcie->port_id);
	if (ret)
		return ret;
...
static const struct dm_pci_ops pcie_dw_x1_ops = {
	.read_config	= pcie_dw_read_config,
	.write_config	= pcie_dw_write_config,
};

static const struct udevice_id pcie_dw_x1_ids[] = {
	{ .compatible = "x1,dwc-pcie" },
	{ }
};

U_BOOT_DRIVER(pcie_dw_x1) = {
	.name			= "pcie_dw_x1",
	.id			= UCLASS_PCI,
	.of_match		= pcie_dw_x1_ids,
	.ops			= &pcie_dw_x1_ops,
	.of_to_plat	= pcie_dw_x1_of_to_plat,
	.probe			= pcie_dw_x1_probe,
	.priv_auto	= sizeof(struct pcie_dw_x1),
};
