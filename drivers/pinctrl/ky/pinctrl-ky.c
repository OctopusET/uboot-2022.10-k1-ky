...
static int ky_pinctrl_set_state(struct udevice *dev, struct udevice *config)
{
	struct ky_pinctrl_priv *priv = dev_get_priv(dev);
	struct ky_pinctrl_soc_info *info = priv->info;
	const struct ky_regs *regs = info->regs;
	const struct ky_pin_conf *pin_conf = info->pinconf;
	...
	pin_size = KY_PIN_SIZE;

	prop = fdt_getprop(gd->fdt_blob, node, "ky,pins", &size);
	if (!prop) {
		dev_err(dev, "No ky,pins property in node %s\n", config->name);
		return -EINVAL;
	}

	if (!size || size % pin_size) {
		dev_err(dev, "Invalid ky,pins property in node %s\n",
			config->name);
		return -EINVAL;
	}
	...
	if (fdtdec_get_int_array(gd->fdt_blob, node, "ky,pins",
				 pin_data, size >> 2)) {
...
const struct pinctrl_ops ky_pinctrl_ops  = {
	.set_state = ky_pinctrl_set_state,
};
		...
		struct ky_pinctrl_soc_info *info)
{
	...
	dev_dbg(dev, "initialized ky pinctrl driver\n");
