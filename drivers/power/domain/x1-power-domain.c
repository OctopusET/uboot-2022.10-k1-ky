...
enum pm_domain_id {
	X1_PMU_VPU_PWR_DOMAIN,
	X1_PMU_GPU_PWR_DOMAIN,
	X1_PMU_LCD_PWR_DOMAIN,
	X1_PMU_ISP_PWR_DOMAIN,
	X1_PMU_AUD_PWR_DOMAIN,
	X1_PMU_GNSS_PWR_DOMAIN,
	X1_PMU_HDMI_PWR_DOMAIN,
	X1_PMU_WKUP_EVENT_PWR_DOMAIN,
};
...
struct ky_x1_pd_platdata {
	struct regmap *regmap[MAX_REGMAP];
	struct pm_domain_desc *desc;
};

static struct pm_domain_desc x1_pm_domain_desc[] = {
	[X1_PMU_VPU_PWR_DOMAIN] = {
		.reg_pwr_ctrl = 0xa8,
		.pm_qos = 12,
		.bit_sleep2 = 3,
		...
		.bit_pwr_stat = 1,
		.bit_hw_pwr_stat = 9,
		.pm_index = X1_PMU_VPU_PWR_DOMAIN,
	},
	...
	[X1_PMU_GPU_PWR_DOMAIN] = {
		...
		.bit_isolation = 1,
		.bit_pwr_stat = 0,
		.pm_index = X1_PMU_GPU_PWR_DOMAIN,
	},
	...
	[X1_PMU_LCD_PWR_DOMAIN] = {
		...
		.bit_hw_pwr_stat = 12,
		.use_hw = 1,
		.pm_index = X1_PMU_LCD_PWR_DOMAIN,
	},
	...
	[X1_PMU_ISP_PWR_DOMAIN] = {
		...
		.bit_pwr_stat = 2,
		.bit_hw_pwr_stat = 10,
		.pm_index = X1_PMU_ISP_PWR_DOMAIN,
	},
	...
	[X1_PMU_AUD_PWR_DOMAIN] = {
		...
		.bit_hw_pwr_stat = 11,
		.use_hw = 1,
		.pm_index = X1_PMU_AUD_PWR_DOMAIN,
	},
	...
	[X1_PMU_GNSS_PWR_DOMAIN] = {
		...
		.bit_pwr_stat = 6,
		.bit_hw_pwr_stat = 14,
		.pm_index = X1_PMU_GNSS_PWR_DOMAIN,
	},
	...
	[X1_PMU_HDMI_PWR_DOMAIN] = {
		...
		.bit_hw_pwr_stat = 15,
		.use_hw = 1,
		.pm_index = X1_PMU_HDMI_PWR_DOMAIN,
	},

	[X1_PMU_WKUP_EVENT_PWR_DOMAIN] = {
		.pm_index = X1_PMU_WKUP_EVENT_PWR_DOMAIN,
	},
};

static const struct udevice_id ky_power_domain_of_match[] = {
	{ .compatible = "ky,x1-pm-domain", .data = (ulong)x1_pm_domain_desc, },
	{ /* sentinel */ }
};

static int ky_power_domain_of_xlate(struct power_domain *pd, struct ofnode_phandle_args *args)
{
	struct ky_x1_pd_platdata *priv = dev_get_priv(pd->dev);
...
static int x1_pd_power_off(struct ky_x1_pd_platdata *skp, struct pm_domain_desc *desc)
{
	unsigned int val;
	int loop;
        unsigned int apcr_per;

	if (desc->pm_index == X1_PMU_WKUP_EVENT_PWR_DOMAIN) {
...
static int x1_pd_power_on(struct ky_x1_pd_platdata *skp, struct pm_domain_desc *desc)
{
	int loop;
	unsigned int val;
	unsigned int apcr_per;
...
static int ky_power_domain_on(struct power_domain *pd)
{
	...
	/* domain_on */
	x1_pd_power_on(priv, pd_priv);

	return 0;
}

static int ky_power_domain_off(struct power_domain *pd)
{
	struct pm_domain_desc *pd_priv = pd->priv;
	struct ky_x1_pd_platdata *priv = dev_get_priv(pd->dev);
	...
	debug("%s(pd=%p, id=%lu)\n", __func__, pd, pd->id);
	/* domain_off */
	x1_pd_power_off(priv, pd_priv);

	return 0;
}

static int ky_power_domain_probe(struct udevice *dev)
{
	int ret;
	struct ky_x1_pd_platdata *priv = dev_get_priv(dev);
...
static struct power_domain_ops ky_power_domain_ops = {
	.on = ky_power_domain_on,
	.off = ky_power_domain_off,
	.of_xlate = ky_power_domain_of_xlate,
};

U_BOOT_DRIVER(ky_pm_domains) = {
	.name = "ky-x1-pm-domains",
	.id = UCLASS_POWER_DOMAIN,
	.of_match = ky_power_domain_of_match,
	.probe = ky_power_domain_probe,
	.priv_auto = sizeof(struct ky_x1_pd_platdata),
	.ops = &ky_power_domain_ops,
};
