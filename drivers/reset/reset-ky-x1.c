...
/*
 * Driver for reset of ky x1 in uboot
 *
 * Copyright (C) 2023 Ky
 */
...
#include <dt-bindings/reset/reset-ky-x1.h>
...
struct ky_reset {
	...
	const struct ky_reset_signal *signals;
};
...
static const struct ky_reset_signal
	x1_reset_signals[RESET_NUMBER_SPL] = {
...
static u32 ky_reset_read(struct ky_reset *reset,
	u32 id)
{
	void __iomem *base;
...
static void ky_reset_write(struct ky_reset *reset, u32 value,
	u32 id)
{
	void __iomem *base;
	switch (reset->signals[id].type) {
...
static void ky_reset_set(struct reset_ctl *rst,
	u32 id, bool assert)
{
	u32 value;
	struct ky_reset *reset = dev_get_priv(rst->dev);

	pr_debug("[RESET]ky_reset_set assert=%d, id=%d \r\n", assert, id);
	value = ky_reset_read(reset, id);
	if(assert == true) {
	...
	ky_reset_write(reset, value, id);
}

static int ky_reset_update(struct reset_ctl *rst, bool assert)
{
#ifdef CONFIG_SPL_BUILD
	...
	ky_reset_set(rst, rst->id, assert);
	return 0;
}

static int ky_reset_assert(struct reset_ctl *rst)
{
	return ky_reset_update(rst, true);
}

static int ky_reset_deassert(struct reset_ctl *rst)
{
	return ky_reset_update(rst, false);
}


static int ky_x1_reset_probe(struct udevice *dev)
{
	struct ky_reset *reset = dev_get_priv(dev);
	...
	reset->signals = x1_reset_signals;
	pr_info("reset driver probe finish \n");
out:
	return 0;
}

const struct reset_ops x1_reset_ops = {
	.rst_assert = ky_reset_assert,
	.rst_deassert = ky_reset_deassert,
};

static const struct udevice_id x1_reset_ids[] = {
	{ .compatible = "ky,x1-reset", },
	{},
};

U_BOOT_DRIVER(x1_reset) = {
	.name		= "ky,x1-reset",
	.id		= UCLASS_RESET,
	.ops		= &x1_reset_ops,
	.of_match = x1_reset_ids,
	.probe		= ky_x1_reset_probe,
	.priv_auto	= sizeof(struct ky_reset),
};
