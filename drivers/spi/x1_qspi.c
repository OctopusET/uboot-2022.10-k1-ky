...
#define x1_QSPI_DEFAULT_CLK_FREQ	26000000
...
static void qspi_set_func_clk(struct x1_qspi *qspi)
{
...
static int qspi_reset(struct x1_qspi *qspi)
{
	uint32_t reg, sr, fr;
	int count = 0;
...
static int qspi_write_sfar(struct x1_qspi *qspi, uint32_t val)
{
	uint32_t fr;
	int count = 0;
 ...
 */
static int qspi_write_rbct(struct x1_qspi *qspi, uint32_t val)
{
	uint32_t fr;
	int count = 0;
...
static void x1_qspi_select_mem(struct x1_qspi *qspi, int chip_select)
{
	u64 size_kb;
...
static void x1_qspi_ahb_read(struct x1_qspi *qspi,
...
static void x1_qspi_fill_txfifo(struct x1_qspi *qspi,
				 const struct spi_mem_op *op)
{
	void __iomem *base = qspi->iobase;
	int i;
...
static int x1_qspi_exec_op(struct spi_slave *slave,
			    const struct spi_mem_op *op)
{
	struct x1_qspi *qspi;
	struct udevice *bus;
	...
	err = x1_qspi_readl_poll_tout(qspi, base + QSPI_SR, mask, 100*1000, QSPI_WAIT_BIT_CLEAR);
	if (err) {
...
static bool x1_qspi_supports_op(struct spi_slave *slave,
				 const struct spi_mem_op *op)
{
	struct x1_qspi *qspi;
	struct udevice *bus;
...
static int x1_qspi_adjust_op_size(struct spi_slave *slave,
...
static int x1_qspi_probe(struct udevice *bus)
{
	struct x1_qspi *host = dev_get_priv(bus);

	return x1_qspi_host_init(host);
}

static int x1_qspi_claim_bus(struct udevice *dev)
{
	struct x1_qspi *qspi;
	struct udevice *bus;
	...
	x1_qspi_select_mem(qspi, slave_plat->cs);

	return 0;
}

static int x1_qspi_set_speed(struct udevice *bus, uint speed)
{
	/* TODO: if need */
	return 0;
}

static int x1_qspi_set_mode(struct udevice *bus, uint mode)
{
	/* TODO: if need */
	return 0;
}

static int x1_qspi_ofdata_to_platdata(struct udevice *bus)
{
	struct x1_qspi *qspi = dev_get_priv(bus);
	int ret;
	struct resource res;
	fdt_addr_t iobase;
	...
	qspi->max_hz = fdtdec_get_int(blob, node, "spi-max-frequency", x1_QSPI_DEFAULT_CLK_FREQ);
...
static const struct dm_spi_ops x1_qspi_ops = {
	.claim_bus	= x1_qspi_claim_bus,
	.set_speed	= x1_qspi_set_speed,
	.set_mode	= x1_qspi_set_mode,
	.mem_ops	= &x1_qspi_mem_ops,
};

static const struct udevice_id x1_qspi_ids[] = {
	{ .compatible = "ky,x1-qspi", },
	{ }
};

U_BOOT_DRIVER(x1_qspi) = {
	.name	= "x1_qspi",
	.id	= UCLASS_SPI,
	.of_match = x1_qspi_ids,
	.ops	= &x1_qspi_ops,
	.of_to_plat = x1_qspi_ofdata_to_platdata,
	.priv_auto = sizeof(struct x1_qspi),
	.probe	= x1_qspi_probe,
};
