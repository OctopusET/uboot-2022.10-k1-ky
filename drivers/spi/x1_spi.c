...
#define X1_APBC_BASE   0xd4015000
#define X1_APBC2_BASE  0xf0610000
#define X1_GPIO_BASE	0xd4019000
...
#define X1_APBC_ADDR(x)		((uint32_t)((X1_APBC_BASE) + (x)))
#define REG_APBC_SSPA0_CLK_RST	X1_APBC_ADDR(APBC_SSPA0) /*Clock/Reset Control Register for SSPA 0*/
#define REG_APBC_SSPA1_CLK_RST	X1_APBC_ADDR(APBC_SSPA1) /*Clock/Reset Control Register for SSPA 1*/
#define REG_APBC_SSP2_CLK_RST	((uint32_t)(X1_APBC2_BASE + APBC_SSP2)) /*Clock/Reset Control Register for SSPA 1*/
#define REG_APBC_SSP3_CLK_RST	X1_APBC_ADDR(APBC_SSP3)  /*Clock/Reset Control Register for SSP 3*/
...
struct x1_spi {
	...
	int flags;
	int n_bytes;
	int (*write)(struct x1_spi *priv);
	int (*read)(struct x1_spi *priv);
	int data_length;
	void *tx;
	void *tx_end;
	void *rx;
...
static bool x1_spi_txfifo_full(struct x1_spi *priv)
{
	return !(readl(priv->base + REG_SSP_STATUS) & BIT_SSP_TNF);
}

int x1_spi_flush(struct x1_spi *priv)
{
...
static int null_writer(struct x1_spi *priv)
{
	u8 n_bytes = priv->n_bytes;

	if (x1_spi_txfifo_full(priv)
		|| (priv->tx == priv->tx_end))
		return 0;
...
static int null_reader(struct x1_spi *priv)
{
	u8 n_bytes = priv->n_bytes;
...
static int u8_writer(struct x1_spi *priv)
{
	if (x1_spi_txfifo_full(priv)
		|| (priv->tx == priv->tx_end))
		return 0;
...
static int u8_reader(struct x1_spi *priv)
{
	while ((readl(priv->base + REG_SSP_STATUS) & BIT_SSP_RNE)
	       && (priv->rx < priv->rx_end)) {
...
void x1_spi_set_selfloop(struct x1_spi *priv, int is_loop)
{
	if (is_loop)
...
static void x1_gpio_set_output(int gpio, int value)
{
	void __iomem *reg;
	...
	reg_gbase = X1_GPIO_BASE + reg_goff;
	bit_no = (gpio) & 0x1f;
...
static void x1_gpio_set_value(int gpio, int value)
{
	void __iomem *reg;
	u32 reg_gbase = 0, reg_goff = 0, bit_no = 0;
	u32 val;
...
static void x1_stop_ssp(struct x1_spi *priv)
{
...
int x1_spi_pio_xfer(struct x1_spi *priv, int len, void *din, unsigned long flags)
{
	if (flags & SPI_XFER_BEGIN) {
		x1_gpio_set_value(priv->cs_gpio, 0);
	}
	...
	if (flags & SPI_XFER_END) {
		x1_gpio_set_value(priv->cs_gpio, 1);
	}
...
static int x1_spi_transfer_config(struct x1_spi *priv)
{
	uint32_t sscr0 = 0;
	u32 top_ctrl;
	u32 fifo_ctrl;
	...
	/* empty read buffer */
	if (x1_spi_flush(priv) == 0) {
		printf("x1 spi flush failed\n");
		return -1;
	}
...
static int x1_spi_claim_bus(struct udevice *dev)
{
	return 0;
}

static int x1_spi_release_bus(struct udevice *dev)
{
	u32 val = 0;
	struct udevice *bus = dev->parent;
	struct x1_spi *priv = dev_get_priv(bus);
	printf("x1 spi release bus\n");
...
static int x1_spi_set_mode(struct udevice *bus, uint mode)
{
	struct x1_spi *priv = dev_get_priv(bus);

	printf("x1 spi set mode = %d.\n", mode);
	priv->mode = mode;

	return 0;
}

static int x1_spi_set_speed(struct udevice *bus, uint speed)
{
	struct x1_spi *priv = dev_get_priv(bus);

	printf("x1 spi set speed = %d\n", speed);
...
static int x1_spi_xfer(struct udevice *dev, unsigned int bitlen,
			   const void *dout, void *din, unsigned long flags)
{
	...
	x1_spi_transfer_config(priv);
	ret = x1_spi_pio_xfer(priv, priv->len, din, flags);

	return ret;
}

static int x1_spi_probe(struct udevice *dev)
{
	struct x1_spi *priv = dev_get_priv(dev);
	int ret;
	...
	x1_gpio_set_output(priv->cs_gpio, 1);
	...
	//x1_spi_set_selfloop(priv, 1);

	return 0;
}

static const struct dm_spi_ops x1_spi_ops = {
	.claim_bus	= x1_spi_claim_bus,
	.release_bus	= x1_spi_release_bus,
	.set_mode = x1_spi_set_mode,
	.set_speed = x1_spi_set_speed,
	.xfer = x1_spi_xfer,
};

static const struct udevice_id x1_spi_ids[] = {
	{ .compatible = "ky,x1-spi" },
	{ }
};

U_BOOT_DRIVER(x1_spi) = {
	.name = "x1_spi",
	.id = UCLASS_SPI,
	.of_match = x1_spi_ids,
	.ops = &x1_spi_ops,
	.priv_auto = sizeof(struct x1_spi),
	.probe = x1_spi_probe,
};
