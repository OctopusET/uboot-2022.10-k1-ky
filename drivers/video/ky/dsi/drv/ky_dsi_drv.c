...
#define KY_DSI_NAME "ky_dsi"
#define KY_DSI_VERSION	"1.0.0"

#define KY_ESC_CLK_DEFAULT	51200000
#define KY_BIT_CLK_DEFAULT	614400000
...
static void dsi_get_advanced_setting(struct ky_dsi_device *dev, struct ky_mipi_info *mipi_info)
{
	struct ky_dsi_advanced_setting *adv_setting = &dev->adv_setting;
...
static void dsi_get_dphy_setting(struct ky_dsi_device *dev)
{
...
static void dsi_open_dphy(struct ky_dsi_device* device_ctx,
								struct ky_mipi_info *mipi_info, bool ready)
{
	struct ky_dphy_ctx *dphy_config = NULL;
	...
	ky_dphy_init(dphy_config);
}

static void dsi_close_dphy(struct ky_dsi_device* device_ctx)
{
	ky_dphy_uninit(&device_ctx->dphy_config);
}

int ky_dsi_open(struct ky_dsi_device* device_ctx, struct ky_mipi_info *mipi_info, bool ready)
{
	int lane_number;
	...
	if(!ready) {
		...
		dsi_enable_lptx_lanes(ky_dsi_lane[lane_number]);
...
int ky_dsi_ready_for_datatx(struct ky_dsi_device* device_ctx, struct ky_mipi_info *mipi_info)
{
	if((NULL == device_ctx) || (NULL == mipi_info)){
		pr_info("%s: Invalid param\n", __func__);
		return -1;
...
struct ky_dsi_driver_ctx dsi_driver_ctx = {
...
struct ky_dsi_device ky_dsi_dev = {0};

int ky_dsi_probe(void)
{
	int ret;

	ky_dsi_dev.driver_ctx = &dsi_driver_ctx;
	ky_dsi_dev.status = DSI_STATUS_UNINIT;
	ky_dsi_dev.id = 0;

	ky_dsi_dev.esc_clk_rate = KY_ESC_CLK_DEFAULT;
	ky_dsi_dev.bit_clk_rate = KY_BIT_CLK_DEFAULT;

	ret = ky_dsi_register_device(&ky_dsi_dev);
	if(ret != 0){
		pr_info("%s: register dsi (%d) device fail!\n", __func__, ky_dsi_dev.id);
		return -1;
	}

	ky_dsi_dev.status = DSI_STATUS_INIT;
