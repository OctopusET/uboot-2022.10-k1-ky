...
#ifndef _KY_VIDEO_TX_H_
#define _KY_VIDEO_TX_H_
...
struct lcd_mipi_panel_info {
	...
	struct ky_mode_modeinfo *ky_modeinfo;
	struct ky_mipi_info *mipi_info;
	struct ky_dsi_cmd_desc *set_power_cmds;
	struct ky_dsi_cmd_desc *set_backlight_value_cmds;
	struct ky_dsi_cmd_desc *set_backlight_pwm_freq_cmds;
	struct ky_dsi_cmd_desc *read_power_cmds;
	struct ky_dsi_cmd_desc *set_id_cmds;
	struct ky_dsi_cmd_desc *read_id_cmds;
	struct ky_dsi_cmd_desc *init_cmds;
	struct ky_dsi_cmd_desc *sleep_out_cmds;
	struct ky_dsi_cmd_desc *sleep_in_cmds;
...
struct video_tx_driver {
	...
	int (*get_modes)(struct video_tx_device *, struct ky_mode_modeinfo *);
...
struct lcd_mipi_tx_data {
	...
	struct ky_panel_priv *priv;
};

struct ky_panel_priv {
	char panel_name[128];
...
int video_tx_get_modes(struct video_tx_device *video_tx,
		       struct ky_mode_modeinfo *modelist);
