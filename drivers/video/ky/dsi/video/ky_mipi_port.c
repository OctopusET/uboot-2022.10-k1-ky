...
{
	struct ky_dsi_rx_buf dbuf;
	uint32_t read_id[3] = {0};
	int i;
	int ret = 0;

	for(i=0;i<1;i++){
		ky_mipi_write_cmds(0, video_tx_client->panel_info->set_id_cmds,
			video_tx_client->panel_info->set_id_cmds_num);

		ret = ky_mipi_read_cmds(0, &dbuf, video_tx_client->panel_info->read_id_cmds,
...
static int lcd_mipi_reset(struct ky_panel_priv *priv)
{
	/* reset lcm */
	if (priv->reset_valid) {
...
static int lcd_mipi_dc_enable(bool power_on, struct ky_panel_priv *priv)
{
	if(power_on){
...
static uint32_t lcd_mipi_readpower(struct lcd_mipi_tx_data *video_tx_client)
{
	...
	ky_mipi_write_cmds(0, video_tx_client->panel_info->set_power_cmds,
		video_tx_client->panel_info->set_power_cmds_num);

	ky_mipi_read_cmds(0, &dbuf, video_tx_client->panel_info->read_power_cmds,
...
static int lcd_mipi_panel_reset(struct video_tx_device *dev)
{
	...
	if (video_tx_client->panel_info->panel_type == LCD_MIPI) {
		ret = ky_mipi_write_cmds(0, video_tx_client->panel_info->init_cmds,
				video_tx_client->panel_info->init_cmds_num);
		if(ret) {
			pr_info("send init cmd fail!\n ");
		}
		ret = ky_mipi_write_cmds(0, video_tx_client->panel_info->sleep_out_cmds,
	...
	ret = ky_mipi_ready_for_datatx(0, video_tx_client->panel_info->mipi_info);
	if (0 != ret) {
		pr_info("lcd_mipi ky_mipi_ready_for_datatx fail!\n ");
		ky_mipi_close(0);
	}
...
static int lcd_mipi_identify(struct video_tx_device *dev)
{
	...
	for(i=0; i<panel_num; i++) {
		...
		if (panel_info->panel_type == LCD_EDP) {
			...
			ret = ky_mipi_open(0, video_tx_client->panel_info->mipi_info, false);
			if(0 != ret) {
		...
		if (!is_panel) {
			...
			lcd_width = video_tx_client->panel_info->ky_modeinfo->xres;
			lcd_height = video_tx_client->panel_info->ky_modeinfo->yres;
			return 1;
		}
	}
	lcd_mipi_dc_enable(false, video_tx_client->priv);
...
static int lcd_mipi_get_modes(struct video_tx_device *dev,
			  struct ky_mode_modeinfo *mode_info)
{
	struct lcd_mipi_tx_data  *video_tx_client =
				video_tx_get_drvdata(dev);
	...
	memcpy(mode_info, (void *)(video_tx_client->panel_info->ky_modeinfo), sizeof(struct ky_mode_modeinfo));
	return 1;
}
...
static int lcd_bl_enable(struct video_tx_device *dev, bool enable)
{
	struct lcd_mipi_tx_data  *video_tx_client =
				video_tx_get_drvdata(dev);
	struct ky_panel_priv *priv = video_tx_client->priv;
	int ret;
...
static int lcd_mipi_client_init(struct ky_panel_priv *priv)
{
...
int lcd_mipi_probe(void)
{
	...
	struct ky_panel_priv *priv = NULL;

	ret = uclass_get_device_by_driver(UCLASS_NOP,
		DM_DRIVER_GET(ky_panel), &dev);
	if (ret) {
		pr_info("ky_panel probe failed %d\n", ret);
		return ret;
	}
...
static const struct udevice_id ky_panel_ids[] = {
        { .compatible = "ky,panel" },
        { }
};

static int ky_panel_of_to_plat(struct udevice *dev)
{

	struct ky_panel_priv *priv = dev_get_priv(dev);
	const char *name;
	int ret;
	...
	if (name) {
		...
		pr_info("ky_panel_of_to_plat panel %s \n", priv->panel_name);
	}
...
U_BOOT_DRIVER(ky_panel) = {
        .name   = "ky-panel",
        .id     = UCLASS_NOP,
        .of_to_plat     = ky_panel_of_to_plat,
        .of_match = ky_panel_ids,
        .priv_auto      = sizeof(struct ky_panel_priv),
};
