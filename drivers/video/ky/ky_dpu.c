...
struct ky_mode_modeinfo hdmi_1080p_modeinfo = {
	.name = "1920x1080-60",
	.refresh = 60,
	.xres = 1920,
	.yres = 1080,
...
static void hdmi_dpu_init(struct ky_mode_modeinfo *hdmi_modeinfo, ulong fbbase)
{
...
static void dsi_dpu_init(struct ky_mode_modeinfo *ky_mode, ulong fbbase)
{
	unsigned int vsync, hsync, vbp, vfp, hbp, hfp, vsp, hsp;
	unsigned int reg_val = 0;

	vsync = ky_mode->vsync_len & 0x3ff;
	hsync = ky_mode->hsync_len & 0x3ff;
	vbp = ky_mode->upper_margin & 0xfff;
	vfp = ky_mode->lower_margin & 0xfff;
	hbp = ky_mode->left_margin & 0xfff;
	hfp = ky_mode->right_margin & 0xfff;
	vsp = ky_mode->hsync_invert ? 0 : 1;
	hsp = ky_mode->hsync_invert ? 0 : 1;
	...
	dsi_dpu_write((void __iomem *)0x18000, (ky_mode->yres << 16) | ky_mode->xres);
	...
	dsi_dpu_write((void __iomem *)0x1808c, (ky_mode->yres << 16) | ky_mode->xres);
	dsi_dpu_write((void __iomem *)0x18090, ky_mode->pix_fmt_out);
	...
	dsi_dpu_write((void __iomem *)0xdb8, ky_mode->xres * 4);
	dsi_dpu_write((void __iomem *)0xdbc, (ky_mode->yres << 16) | ky_mode->xres);
	dsi_dpu_write((void __iomem *)0xdc0, 0x0);
	dsi_dpu_write((void __iomem *)0xdc4, ((ky_mode->yres - 1) << 16) | (ky_mode->xres - 1));
	dsi_dpu_write((void __iomem *)0xdf0, 0x4);
	/* PP3 will impact display light */
	dsi_dpu_write((void __iomem *)0x4c00, (ky_mode->xres << 8) | 0x01);
	dsi_dpu_write((void __iomem *)0x4c04, ky_mode->yres);
	...
	dsi_dpu_write((void __iomem *)0x4c4c, (ky_mode->xres - 1) << 16);
	dsi_dpu_write((void __iomem *)0x4c50, ky_mode->yres - 1);
...
static int ky_display_init(struct udevice *dev, ulong fbbase, ofnode ep_node)
{
	// struct video_uc_plat *uc_plat = dev_get_uclass_plat(dev);
	struct video_priv *uc_priv = dev_get_uclass_priv(dev);
	// struct ky_dpu_priv *priv = dev_get_priv(dev);
	...
	struct ky_mode_modeinfo *ky_mode = NULL;
	...
	if(dpu_id == DPU_MODE_HDMI)
	{
		...
		ky_mode = &fbi.mode;
		uc_priv->xsize = ky_mode->xres;
		uc_priv->ysize = ky_mode->yres;
...
static int ky_dpu_probe(struct udevice *dev)
{
	...
	*/
	...
	for (node = ofnode_first_subnode(port);
		...
		ret = ky_display_init(dev, plat->base, node);
		if (ret)
...
static int ky_dpu_remove(struct udevice *dev)
{
	return 0;
}

struct ky_dpu_driverdata dpu_driverdata = {
	.features = DPU_FEATURE_OUTPUT_10BIT,
};

static const struct udevice_id ky_dc_ids[] = {
	{ .compatible = "ky,dpu",
	  .data = (ulong)&dpu_driverdata },
	{ }
};

static const struct video_ops ky_dpu_ops = {
};

int ky_dpu_bind(struct udevice *dev)
{
	struct video_uc_plat *plat = dev_get_uclass_plat(dev);

	pr_debug("%s,%d,plat->size = %d plat->base 0x%lx\n",__func__,__LINE__,plat->size, plat->base);

	plat->size = 4 * (CONFIG_VIDEO_KY_MAX_XRES *
			  CONFIG_VIDEO_KY_MAX_YRES);

	return 0;
}

U_BOOT_DRIVER(ky_dpu) = {
	.name	= "ky_dpu",
	.id	= UCLASS_VIDEO,
	.of_match = ky_dc_ids,
	.ops	= &ky_dpu_ops,
	...
	.priv_auto	= sizeof(struct ky_dpu_priv),
};
