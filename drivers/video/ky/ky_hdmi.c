...
static int hdmi_get_plug_in_status(struct dw_hdmi *hdmi)
{
	...
	pr_debug("%s() \n", __func__);
	value = readl(hdmi_addr + KY_HDMI_PHY_STATUS) & KY_HDMI_PHY_HPD;
...
{
	struct ky_hdmi_priv *priv = dev_get_priv(dev);

	return dw_hdmi_read_edid(&priv->hdmi, buf, buf_size);
}

static int ky_hdmi_of_to_plat(struct udevice *dev)
{
	return 0;
}

static int ky_hdmi_probe(struct udevice *dev)
{
	struct ky_hdmi_priv *priv = dev_get_priv(dev);
...
static const struct dm_display_ops ky_hdmi_ops = {
	.read_edid = hdmi_read_edid,
	.enable = hdmi_enable,
};

static const struct udevice_id ky_hdmi_ids[] = {
	{ .compatible = "ky,hdmi" },
	{ }
};

U_BOOT_DRIVER(ky_hdmi) = {
	.name = "ky_hdmi",
	.id = UCLASS_DISPLAY,
	.of_match = ky_hdmi_ids,
	.ops = &ky_hdmi_ops,
	.of_to_plat = ky_hdmi_of_to_plat,
	.probe = ky_hdmi_probe,
	.priv_auto	= sizeof(struct ky_hdmi_priv),
};
