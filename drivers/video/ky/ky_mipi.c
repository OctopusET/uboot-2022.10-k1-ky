...
#include "ky_mipi.h"
#include "./dsi/include/ky_dsi_common.h"


static int ky_mipi_dsi_enable(struct udevice *dev,
		       const struct display_timing *timing)
{
	ofnode node, timing_node;
	int val;
	// struct ky_mipi_priv *priv = dev_get_priv(dev);
...
static int ky_mipi_phy_enable(struct udevice *dev)
{
	// struct ky_mipi_priv *priv = dev_get_priv(dev);
	return 0;
}
			  ...
			  const struct display_timing *timing)
{
	...
	ret = ky_mipi_dsi_enable(dev, timing);
	if (ret) {
		pr_debug("%s: ky_mipi_dsi_enable() failed (err=%d)\n",
		      __func__, ret);
		return ret;
	}

	/* Config and enable mipi phy */
	ret = ky_mipi_phy_enable(dev);
	if (ret) {
		pr_debug("%s: ky_mipi_phy_enable() failed (err=%d)\n",
		      __func__, ret);
		return ret;
	}
...
static int ky_mipi_dsi_of_to_plat(struct udevice *dev)
{
	lcd_mipi_probe();
	return 0;
}

static int ky_mipi_dsi_probe(struct udevice *dev)
{
	struct ky_mipi_priv *priv = dev_get_priv(dev);
...
static const struct dm_display_ops ky_mipi_dsi_ops = {
	.read_timing = mipi_dsi_read_timing,
	.enable = mipi_dsi_display_enable,
};

static const struct udevice_id ky_mipi_dsi_ids[] = {
	{ .compatible = "ky,mipi-dsi" },
	{ }
};

U_BOOT_DRIVER(ky_mipi_dsi) = {
	.name = "ky_mipi_dsi",
	.id = UCLASS_DISPLAY,
	.of_match = ky_mipi_dsi_ids,
	.ops = &ky_mipi_dsi_ops,
	.of_to_plat = ky_mipi_dsi_of_to_plat,
	.probe = ky_mipi_dsi_probe,
	.priv_auto	= sizeof(struct ky_mipi_priv),
};
