...
{
	struct ky_wdt_priv *priv = dev_get_priv(dev);
	if (!priv) {
...
static int ky_wdt_reset(struct udevice *dev)
{
	struct ky_wdt_priv *priv = dev_get_priv(dev);
	if (!priv) {
		pr_err("Failed to get private data\n");
...
static int ky_wdt_start(struct udevice *dev, u64 timeout_ms, ulong flags)
{
	struct ky_wdt_priv *priv = dev_get_priv(dev);
	if (!priv) {
		pr_err("Failed to get private data\n");
...
static int ky_wdt_stop(struct udevice *dev)
{
	struct ky_wdt_priv *priv = dev_get_priv(dev);
	if (!priv) {
		pr_err("Failed to get private data\n");
...
static int ky_wdt_remove(struct udevice *dev)
{
	return ky_wdt_stop(dev);
}

static int ky_wdt_expire_now(struct udevice *dev, ulong flags)
{
	initialize_wdt(dev, 0);
	return 0;
}

static int ky_wdt_probe(struct udevice *dev)
{
	int ret;
	struct ky_wdt_priv *priv = malloc(sizeof(*priv));
	if (!priv)
		return -ENOMEM;
...
static struct wdt_ops ky_wdt_ops = {
	.start = ky_wdt_start,
	.stop = ky_wdt_stop,
	.reset = ky_wdt_reset,
	.expire_now = ky_wdt_expire_now,
};

static const struct udevice_id ky_wdt_ids[] = {
	{ .compatible = "ky,x1-wdt" },
	{ /* sentinel */ }
};

U_BOOT_DRIVER(ky_wdt) = {
	.name = "ky_wdt",
	.id = UCLASS_WDT,
	.of_match = ky_wdt_ids,
	.probe = ky_wdt_probe,
	.ops = &ky_wdt_ops,
	.remove = ky_wdt_remove,
