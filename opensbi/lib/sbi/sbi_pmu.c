...
#ifdef CONFIG_PLATFORM_KY_X1
static inline int ky_mhpmevent_inhibit_flags_are_invalid(uint64_t mhpmevent_val)
{
...
#endif /* CONFIG_PLATFORM_KY_X1 */

static void pmu_update_inhibit_flags(unsigned long flags, uint64_t *mhpmevent_val)
{
#ifdef CONFIG_PLATFORM_KY_X1
	if (ky_mhpmevent_inhibit_flags_are_invalid(*mhpmevent_val))
		return;
#endif
