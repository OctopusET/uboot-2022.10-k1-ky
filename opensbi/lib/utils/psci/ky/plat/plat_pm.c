...
static int ky_pwr_domain_on(u_register_t mpidr)
{
	/* wakeup the cpu */
	if (ky_core_enter_c2(mpidr)) {
		ky_wakeup_cpu(mpidr);
	} else {
...
static void ky_pwr_domain_on_finish(const psci_power_state_t *target_state)
{
...
static int ky_pwr_domain_off_early(const psci_power_state_t *target_state)
{
...
static void ky_pwr_domain_off(const psci_power_state_t *target_state)
{
	unsigned int hartid = current_hartid();

	if (CLUSTER_PWR_STATE(target_state) == PLAT_MAX_OFF_STATE) {
		/* power-off cluster */
		ky_cluster_off(hartid);
        }
...
static void ky_pwr_domain_pwr_down_wfi(const psci_power_state_t *target_state)
{
	int hstate;
...
static void ky_pwr_domain_on_finish_late(const psci_power_state_t *target_state)
{
	ky_deassert_cpu();
}

static int _ky_validate_power_state(unsigned int power_state,
...
static int ky_validate_power_state(unsigned int power_state,
                            psci_power_state_t *req_state)
{
        int rc;

        rc = _ky_validate_power_state(power_state, req_state);

        return rc;
}

static void ky_pwr_domain_suspend(const psci_power_state_t *target_state)
{
	unsigned int hartid = current_hartid();
	...
	if (SYSTEM_PWR_STATE(target_state) == ARM_LOCAL_STATE_OFF) {
		...
		csr_write(CSR_TCMCFG, 0);
#endif
		if (!ky_cluster_enter_m2(PLATFORM_MAX_CPUS_PER_CLUSTER)) {
...
static void ky_pwr_domain_suspend_finish(const psci_power_state_t *target_state)
{
...
static void ky_pwr_domain_suspend_pwrdown_early(const psci_power_state_t *target_state)
{
	csr_clear(CSR_MIE, MIP_SSIP | MIP_MSIP | MIP_STIP | MIP_MTIP | MIP_SEIP | MIP_MEIP);
}

static void ky_get_sys_suspend_power_state(psci_power_state_t *req_state)
{
	int i;
...
static const plat_psci_ops_t ky_psci_ops = {
	.cpu_standby = NULL,
	.pwr_domain_on = ky_pwr_domain_on,
	.pwr_domain_on_finish = ky_pwr_domain_on_finish,
	.pwr_domain_off_early	= ky_pwr_domain_off_early,
	.pwr_domain_off = ky_pwr_domain_off,
	.pwr_domain_pwr_down_wfi = ky_pwr_domain_pwr_down_wfi,
	.pwr_domain_on_finish_late = ky_pwr_domain_on_finish_late,
	.validate_power_state = ky_validate_power_state,
	.pwr_domain_suspend = ky_pwr_domain_suspend,
	.pwr_domain_suspend_pwrdown_early = ky_pwr_domain_suspend_pwrdown_early,
	.pwr_domain_suspend_finish = ky_pwr_domain_suspend_finish,
	.get_sys_suspend_power_state = ky_get_sys_suspend_power_state,
};

int plat_setup_psci_ops(uintptr_t sec_entrypoint, const plat_psci_ops_t **psci_ops)
{
	*psci_ops = &ky_psci_ops;
