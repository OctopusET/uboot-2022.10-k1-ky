...
#elif defined(CONFIG_PLATFORM_KY_K1PRO)
 ...
 */
static int ky_k1_early_init(bool cold_boot, const struct fdt_match *match)
{
	if (cold_boot) {
		/* initiate cci */
...
static int ky_hart_start(unsigned int hartid, unsigned long saddr)
{
...
static int ky_hart_suspend(unsigned int suspend_type)
{
	psci_cpu_suspend(suspend_type, 0, 0);
	return 0;
}

static void ky_hart_resume(void)
{
	psci_warmboot_entrypoint();
}

static const struct sbi_hsm_device ky_hsm_ops = {
	.name		= "ky-hsm",
	.hart_start	= ky_hart_start,
	.hart_stop	= ky_hart_stop,
	.hart_suspend	= ky_hart_suspend,
	.hart_resume	= ky_hart_resume,
};

static int ky_system_suspend_check(u32 sleep_type)
{
	return sleep_type == SBI_SUSP_SLEEP_TYPE_SUSPEND ? 0 : SBI_EINVAL;
}

static int ky_system_suspend(u32 sleep_type, unsigned long mmode_resume_addr)
{
...
static struct sbi_system_suspend_device ky_system_suspend_ops = {
	.name = "ky-system-suspend",
	.system_suspend_check = ky_system_suspend_check,
	.system_suspend = ky_system_suspend,
};
#endif
 ...
 */
static int ky_k1_final_init(bool cold_boot, const struct fdt_match *match)
{
#ifdef CONFIG_ARM_PSCI_SUPPORT
	/* for clod boot, we build the cpu topology structure */
	if (cold_boot) {
		sbi_hsm_set_device(&ky_hsm_ops);
		/* register system-suspend ops */
		sbi_system_suspend_set_device(&ky_system_suspend_ops);
		return psci_setup();
	}
#endif
...
static bool ky_cold_boot_allowed(u32 hartid, const struct fdt_match *match)
{
	/* enable core snoop */
...
static const struct fdt_match ky_k1_match[] = {
	{ .compatible = "ky,k1-pro" },
	{ .compatible = "ky,x1" },
	{ .compatible = "ky,x1" },
	{ .compatible = "ky,k1" },
	{ },
...
const struct platform_override ky_k1 = {
	.match_table = ky_k1_match,
	.early_init = ky_k1_early_init,
	.final_init = ky_k1_final_init,
	.cold_boot_allowed = ky_cold_boot_allowed,
};
